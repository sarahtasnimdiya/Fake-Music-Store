...
function pickFrom(f, arr) {
  return arr[f.number.int({ min: 0, max: arr.length - 1 })];
}

function fill(f,t,ctx, locale){
  ...
  return t.replace(/\{(\w+)\}/g,(_,k)=>{
    switch(k){
      ...
      case 'Nouns':return wl ? pw(wl.nouns) + 'গুলো' : cap(pluralizeEn(f.word.noun()));case 'nouns':return wl ? pw(wl.nouns) + 'গুলো' : pluralizeEn(f.word.noun());
      ...
      case 'verbPast': {
        ...
        const v = f.word.verb();
        return cap(v.endsWith('e') ? v + 'd' : v + 'ed');
      }
      case 'verbIng': {
        if (wl) return pickFrom(f, wl.verbs) + 'ছি'; 
        const v = f.word.verb();
      ...
      case 'artist':return ctx.artist||'';
      case 'genre':return ctx.genre||'';
      default:return '';
    }
  });
}
...
function generateReview(seed,index,regionCode,record){
  ...
  return fill(f,t,{title:record.title,artist:record.artist,genre:record.genre}, locale);
}
